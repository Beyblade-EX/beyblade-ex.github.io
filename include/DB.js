...
const DB = {
    ...
    discard: (db, handler) => {
        ...
        return DB.transfer.out().then(() => new Promise(res => {
            DB.db?.close();
            Object.assign(indexedDB.deleteDatabase(db.name || db), {        
    ...
    transfer: {
        out: () => DB.get.all('user').then(data => localStorage.setItem('user', JSON.stringify(data))).catch(() => {}),
        in: () => DB.put('user', JSON.parse(localStorage.getItem('user') ?? '[]').map((item, i) => ({[`sheet-${i+1}`] : item})))
    ...
    ,
    setup ({transaction}) {
        ['user'].map(s => DB.db.createObjectStore(s));
